...
define('DATABASE', '/Users/hasan_azimi0/Sites/phpLiteAdmin/db/new.sqlite3');
...
function main()
{
    ...
    if( file_exists(DATABASE) ) {
        message('Database (%s) already exists', DATABASE);
    } else {
        try {
            $db = new SQLite3(DATABASE);
            message('Database (%s) successfully created', DATABASE);
        } catch(Exception $e) {
            message('SQLite3 could not create database (%s)', $e->getMessage());
        }
    }
}
